...
- **Веб-панель** — очередь, история, содержимое писем, транспорты, проекты, ключи, шаблоны,
  ...
  Вход по логину и паролю, пользователей может быть сколько угодно, а что кому доступно —
  решает роль: администратор видит всё, обычный пользователь — только свои проекты,
  транспорты, шаблоны и письма.
...
При первом открытии панель попросит завести пользователя — он получит роль администратора.
Завести заранее: `php bin/mailer user:create admin --role="Администратор"`.
...
routes/        карта адресов: api.php и ui.php
