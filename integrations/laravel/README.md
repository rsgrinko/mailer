...
(адрес, ключ, метка), ответ `/health`, письмо через API синхронно (ошибка
отправки видна сразу же), письмо через почтовый транспорт Laravel и список
последних писем проекта с их статусами.
