...
- `transports` — профили отправки: тип (`smtp|sendmail|log|null|failover|roundrobin`),
  ...
  Меняется только адрес, имя отправителя остаётся из письма. В базе лежит адрес от клиента,
  поэтому о подмене пишется событие `sender` — по нему карточка письма и показывает,
  с какого адреса письмо ушло на самом деле.
