...
**База для тестов всегда своя.** Раннер поднимает SQLite в памяти и настройки `DB_*`
из `.env` не читает вовсе: тесты заводят и удаляют записи, а `AuthTest` сносит вообще
всех пользователей панели — на боевой базе это потеря данных. Прогнать те же тесты на
MySQL можно, но только на отдельной пустой базе из `TEST_DB_*` (см. `.env.example`);
совпадение с `DB_DATABASE` раннер не пропускает и останавливается.
...
Предупреждения PHP в тестах приравнены к ошибке: «Undefined array key» во вьюхе роняет
тест, а не уезжает в вывод. Уборка за тестом вешается на `afterTests(...)` — она идёт
после всего прогона, даже если тест упал.
