...
1. PHP-проекты — через мини-SDK (`sdk/MailerClient.php`, один файл, `require` и всё).
...
public/index.php     единая точка входа: /api/v1/... и /ui/... (панель без своей
                     авторизации — basic auth вешается на nginx)
...
sdk/                 мини-SDK для PHP-проектов + примеры
...
- `counters` — счётчики для rate limit (ключ + окно).
...
`/ui/` — страницы без JS-фреймворков и без авторизации (предполагается basic auth на
уровне nginx): дашборд со статистикой, очередь и история писем с фильтрами, карточка письма
(заголовки, HTML-превью в песочнице, текст, raw, события, вложения), управление транспортами,
проектами, шаблонами, просмотр вебхуков и логов, действия «повторить», «отменить»,
